...
BricksSpawner.prototype.createBrickSpots = function() {
    ...
    // famobi.log('Spawned ' + this.entity.brickSpots.length + " brick spots on " + this.entity.name);
};
...
Stickman.prototype.update = function(dt) {
    ...
    this.generateBricksWhenBackpackIsEmptyAndNoOnesOnThePlatform(dt);
...
Stickman.prototype._initializeEntityProperties = function() {
    ...
    Object.defineProperty(this.entity, "color", {
        ...
        set(value) {
            this._color = value;     
            if(this.stickmanModel && this.stickmanModel.model) this.stickmanModel.model.model.meshInstances.forEach(mesh => mesh.material = this._app.colorsStorage.getMaterial(this._color));
            if(this.rigidbody) {
...
Stickman.prototype.generateBricksWhenBackpackIsEmptyAndNoOnesOnThePlatform = function(dt) {
    if(GameSession.getInstance().sessionStarted && GameSession.getInstance().sessionActive && !GameSession.getInstance().sessionPaused) {
        if(this.app.frame % 60 === 0 && this.entity.alive && this.entity.backpack.capacity === 0) {
            if(this.entity.floor && this.entity.floor.isPlatform && this.entity.floor.getBrickSpotsWithColoredBricks(this.entity.color).length === 0) {
                this.entity.floor.fire(EventTypes.Platform.GENERATE_BRICK_AT_EMPTY_SPOT, this.entity.color);    
            }
        }
    }
};
...
Stairs.prototype._assignEntityMethods = function () {
    ...
    this.entity.isSecondLastStair = function (stair) {
        return stair.stairs === this && stair.index === this.stairs.length - 2;
    }.bind(this.entity);
...
Stairs.prototype.onUpdateBlockerPosition = function (stickman) {
    ...
    if (!stair) {
        famobi.log('onUpdateBlockerPosition: no current stair for stickman ' + stickman.color);
        return;
    }
...
Stairs.prototype.onStickmanEnteredStair = function (stickman, stair) {
    ...
    if (stair.stairs.isPassed(stickman.color) && stair.isLast() && !cheatEnabled) {
        if(movementDirection === Constants.StairsMovementDirection.DOWN) {
            const prevStair = stair.getPrev();
            if(prevStair) {
                famobi.log('An attempt to step at already passed stairs by [' + stickman.color + '] has been prevented');
                this.updateBlockerPosition(stickman, prevStair);
                return;
            } 
        } else {
            this.disableBlocker(stickman.color);    
...
Stair.prototype._assignEntityMethods = function() {
    ...
    /* is last stair? */
    this.entity.isSecondLast  = function() {
        return this.stairs.isSecondLastStair(this);
    }.bind(this.entity);
...
Aiplayer.prototype.attackStrongestBot = function(strongestBot) {
    ...
    // famobi.log(this.entity.color + ' attacks strongest bot ' + strongestBot.color);
...
Aiplayer.prototype.log = function(...data) {
    if(GameConfig.getAttribute('debug', 'logAIDecisions')) {
        // famobi.log(`[${this.entity.color}]: `, ...data);
    }
};
...
class SkinsWindow extends CloseableWindow {
    ...
    _initComponents() {
        ...
        this.extraCoinsUnavailableButton = this.entity.findByName('ExtraCoinsUnavailalbeButton');
        
        /* texts */
        this.extraCoinsRewardAmount = this.extraCoinsButton.findByName('ExtraCoinsRewardAmount');
        this.extraCoinsRewardAmount.element.text = '+' + GameConfig.getAttribute('gameplay', 'skinsExtraCoinsForWatchingAVideoAmount');
        this.extraCoinsUnavailableRewardAmount = this.extraCoinsUnavailableButton.findByName('ExtraCoinsRewardAmount');
        this.extraCoinsUnavailableRewardAmount.element.text = '+' + GameConfig.getAttribute('gameplay', 'skinsExtraCoinsForWatchingAVideoAmount');
    ...
    _onShow() {
        ...
        if (Apicontroller.hasRewardedVideo()) {
            this.extraCoinsUnavailableButton.enabled = false;
            ...
            this.extraCoinsUnavailableButton.enabled = true;
            this.extraCoinsUnavailableButton.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY);
        }
    ...
    update(dt) {
        if(!Apicontroller.hasRewardedVideo()) {
            this.extraCoinsButton.enabled = false;
            if(!this.extraCoinsUnavailableButton.enabled) {
                this.extraCoinsUnavailableButton.enabled = true;
                // this.extraCoinsUnavailableButton.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY);
            }
        } else {
            ...
            this.extraCoinsUnavailableButton.enabled = false;
            if(!this.extraCoinsButton.enabled) {
                this.extraCoinsButton.enabled = true;
                if(this._extraCoinsOfferAppeared) {
                    this._extraCoinsOfferAppeared = false;
                    this.extraCoinsButton.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY);
                }
            }
...
class VictoryWindow extends BaseWindow {
    ...
    _initComponents() {
        ...
        this.doubleRewardButtonUnavailable = this.entity.findByName('DoubleRewardButtonUnavailable');
        ...
        this.pickedUpBricks = this.entity.findByName('PickedUpBricks');
        this.pickedUpBricksRewardAmount = this.entity.findByName('PickedUpBricksRewardAmount');
        ...
        this.watchVideoRewardAmount = this.doubleRewardButton.findByName('WatchVideoRewardAmount');
        this.watchVideoRewardAmountUnavailable = this.doubleRewardButtonUnavailable.findByName('WatchVideoRewardAmount');
    ...
    _onShow() {
        ...
        this.pickedUpBricks.element.text = '' + LevelStatsManager.getInstance().getPickedUpBricks();    
        ...
        this.watchVideoRewardAmountUnavailable.element.text = '' + this._levelExtraReward;    

        this.doubleRewardButton.enabled = false;
        this.doubleRewardButtonUnavailable.enabled = false;
        ...
        const pickedUpBricksReward = LevelStatsManager.getInstance().getLiveScore();
        const pickedUpBricksTweenDuration = pc.math.clamp(pickedUpBricksReward / 100, 0.15, 0.75);
        if(pickedUpBricksReward > 0) {
            this.pickedUpBricksRewardAmount.element.text = '+' + pickedUpBricksReward;
            Utils.tweenText(this.pickedUpBricksRewardAmount, 0, pickedUpBricksReward, pickedUpBricksTweenDuration, 0.4, pc.Linear, true, "+");
        } else {
            this.pickedUpBricksRewardAmount.element.text = '+0';
        }
        ...
        if(levelTimeReward > 0) {
            ...
            Utils.tweenText(this.passTimeRewardAmount, 0, levelTimeReward, levelTimeTweenDuration, 0.4 + pickedUpBricksTweenDuration + 0.075, pc.Linear, true, "+");
        ...
        if(extraBricksReward > 0) {
            ...
            Utils.tweenText(this.extraBricksRewardAmount, 0, extraBricksReward, pc.math.clamp(extraBricksReward / 50, 0.15, 0.75), 0.4 + levelTimeTweenDuration + pickedUpBricksTweenDuration + 0.075 * 2, pc.Linear, true, "+");
        } else {
        ...
        setTimeout(() => {
            Promise.all([
                ...
                window.famobi_analytics.trackEvent(
                    ...
                    {
                        levelName: `${LevelController.getInstance().getCurrentLevelIndex() + 1}`,
                        levelScore: Math.floor(LevelStatsManager.getInstance().getLevelScore())
                    }
                ),
    ...
    _showRewardButtons() {
        ...
        this.doubleRewardButtonUnavailable.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY);
    ...
    update() {
        if(!this.rewardClaimed && this.buttonsReady) {
            this.doubleRewardButton.enabled = Apicontroller.hasRewardedVideo();
            this.doubleRewardButtonUnavailable.enabled = !this.doubleRewardButton.enabled;
        } 
    }
    ...
    onContinueClicked() {
        ...
        this.app.fire(EventTypes.SHOW_TRANSITION_SCREEN, 0.175, () => {
            this.app.fire(EventTypes.LOAD_NEXT_LEVEL);//this.app.fire(EventTypes.RELOAD_LEVEL);
...
class ReviveWindow extends BaseWindow {
    ...
    _initComponents() {
        ...
        this.reviveUnavailableButton = this.entity.findByName('ReviveUnavailableButton');
        this.continueButton = this.entity.findByName('ContinueButton');
    ...
    update(dt) {
        if(Apicontroller.hasRewardedVideo()) {
            this.reviveButton.enabled = true;
            this.reviveUnavailableButton.enabled = false;
        } else {
            this.reviveButton.enabled = false;
            this.reviveUnavailableButton.enabled = true;
        }
    }
...
GameplayController.prototype.restartGame = function (dt) {

    if (this.entity.playerStickman) {
        if (this.entity.playerStickman.backpack) {
            const bricks = this.entity.playerStickman.backpack.bricks;
            for (let i = bricks.length - 1; i > -1; i--) {
                bricks[i].reparent(this.app.root);
                bricks[i].destroy();
            }
        }
        this.entity.playerStickman.destroy();
    }

    if (this.entity.stickmansAI) {
        this.entity.stickmansAI.forEach(s => s.destroy());
    }
    ...
    setTimeout(() => doAPIHandshake(() => {
        ...
        if (isForcedMode()) {
            const forcedModeProperties = getForcedModeProperties();
            if (forcedModeProperties.state.initial_bricks > 0) {
                setTimeout(() => pc.Application.getApplication().fire(EventTypes.AWARD_PLAYER_STICKMAN_WITH_BRICKS, forcedModeProperties.state.initial_bricks), 100);
            }
        }
    }), 0);
};
...
LevelController.prototype.selectNextLevel = function() {
    ...
    if(nextLevelIndex >= this.getTotalLevels()) {
        ...
        this.setCurrentLevelIndex(nextLevelIndex);
    }
    this.app.fire(EventTypes.SAVE_APP);
};
...
LevelController.prototype.setCurrentLevelIndex = function(index) {
    LevelController.CURRENT_LEVEL_INDEX = Math.min(index, this.getTotalLevels() - 1);
};
...
StickmanLifeStatus.prototype.onEnterPlatform = function (platform) {
    ...
    // famobi.log('%c Stickman ' + this.entity.color + ' has entered platform ' + platform.name, 'background: #222; color: #bada55');
};
...
(function () {
    ...
    window.famobi.showInterstitialAd = window.famobi.showInterstitialAd || function () { console.log('Shwing an ad...'); return Promise.resolve(); };
    window.famobi.hasRewardedAd = window.famobi.hasRewardedAd || function () { return true; };
    window.famobi.rewardedAd = window.famobi.rewardedAd || function (callback) { setTimeout(() => { console.log("Watching a rewarded video..."); callback({ rewardGranted: true }); }, 1000); };
    ...
    window.famobi.triggerRequest = function (param, ...args) {
        ...
        if (key === 'forced_mode') {
            return {
                ...
                "override": {
                    ...
                    "num_enemies": 1,
                    "player_color": "red",
                    "player_skin": "characterPlaguemask",
                    "brick_skin": "brickBone"
                }
            };
        } else {
            return {};
...
LocalStorageController.getActualSaveData = function() {    
    ...
    if(currentSaveData[Constants.Storage.CURRENT_LEVEL] === currentSaveData[Constants.Storage.MAX_UNLOCKED_LEVEL] - 1) {
        currentSaveData[Constants.Storage.CURRENT_LEVEL] = currentSaveData[Constants.Storage.MAX_UNLOCKED_LEVEL];
    }
...
LevelStatsManager.prototype.getLevelReward = function() {
    return this.getLevelTimeReward() + this.getExtraBricksReward() + this.getLiveScore();
};


LevelStatsManager.prototype.getLevelScore = function() {
    return this.getLevelReward();
};

LevelStatsManager.prototype.getLiveScore = function() {
    return this._bricksPickedUp * GameConfig.getAttribute('gameplay', 'rewardPerBrick');
};

LevelStatsManager.prototype.getPickedUpBricks = function() {
    return this._bricksPickedUp;
};
...
BricksCache.prototype.onAppLoaded = function() {
    if(this.app.colorsStorage.getColorsList().length === 0) {
