...
StopPropagation);elem.addEventListener("pointerrawupdate",StopPropagation);elem.addEventListener("pointerup",StopPropagation);elem.addEventListener("mousedown",StopPropagation);elem.addEventListener("mouseup",StopPropagation);elem.addEventListener("keydown",StopPropagation);elem.addEventListener("keyup",StopPropagation);elem.addEventListener("click",()=>this._PostToRuntimeElementMaybeSync("click",elementId));elem.addEventListener("change",()=>this.PostToRuntimeElement("change",elementId,{"value":parseFloat(elem.value)}));
elem.addEventListener("input",()=>this.PostToRuntimeElement("input",elementId,{"value":parseFloat(elem.value)}));if(e["id"])elem.id=e["id"];if(e["className"])elem.className=e["className"];this.UpdateState(elem,e);return elem}UpdateState(elem,e){elem.max=e["max"];elem.min=e["min"];elem.step=e["step"];elem.value=e["value"];elem.disabled=!e["isEnabled"];elem.title=e["title"]}};self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)};
